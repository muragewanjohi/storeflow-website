 ...
 *
 * Default is a **root-relative** path so it resolves on whatever host serves the storefront (local, Vercel
 * preview `*.vercel.app`, or `*.dukanest.com`) without baking `localhost` or a preview hostname into the DB.
 *
 * Override with `NEXT_PUBLIC_ONBOARDING_PLACEHOLDER_IMAGE_URL` (absolute URL for emails / external consumers).
 */

/**
 * POS bootstrap payload — the sellable catalogue + the store settings needed
 * to compute totals and render a receipt. Shared by the mobile route
 * (GET /api/v1/mobile/pos/bootstrap) and the web route
 * (GET /api/dashboard/pos/bootstrap); the web dashboard page also calls this
 * directly server-side for the initial render.
 *
...
import type { PosBootstrapPayload } from '@/lib/pos/bootstrap-types';

export type { PosBootstrapPayload } from '@/lib/pos/bootstrap-types';
...
export async function loadPosBootstrap(
  ...
  const [productRows, settings, tumiziConfig] = await Promise.all([
    ...
    getStaticOptions(tenant.id, SETTINGS_KEYS),
    getTumiziTenantConfigByTenantId(tenant.id).catch(() => null),
  ]);
  ...
  const taxRate = settings.default_tax_rate
    ? parseFloat(settings.default_tax_rate)
    : null;
    ...
    settings: {
      ...
      payments: {
        cash_enabled:
          ...
          settings.payment_cash_enabled === null,

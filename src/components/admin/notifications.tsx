 ...
 * 
 * Bell icon with dropdown showing notifications for landlord/admin dashboard
 */
...
function getReadNotificationIds(): Set<string> {
  ...
  const stored = localStorage.getItem('read_admin_notifications');
...
function saveReadNotificationIds(ids: Set<string>) {
  ...
  localStorage.setItem('read_admin_notifications', JSON.stringify(Array.from(ids)));
}

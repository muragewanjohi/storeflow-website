/**
 * Create Landlord Support Ticket Client Component
 * 
 * Form for tenant admins to create a support ticket for the landlord
 */
...
interface CreateLandlordTicketClientProps {
  tenant: Tenant;
}

export default function CreateLandlordTicketClient({
  tenant,
}: Readonly<CreateLandlordTicketClientProps>) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create ticket (attachments will be added as initial message if needed)
      ...
      if (attachmentUrls.length > 0 && data.ticket?.id) {
        await fetch(`/api/admin/support/tickets/${data.ticket.id}/messages`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            message: description, // Use description as initial message
      ...
      router.push(`/dashboard/support/landlord-tickets/${data.ticket.id}`);
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto py-8 max-w-2xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Contact Platform Support</h1>
        <p className="text-muted-foreground mt-2">
          Create a support ticket for platform-related issues, billing questions, or feature requests.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Fill out the form below to submit a support request to the platform team
          </CardDescription>
        </CardHeader>
        <CardContent>
